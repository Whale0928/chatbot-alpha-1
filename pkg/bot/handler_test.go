...
func Test_미팅_시작_시_sendSticky는_초기_컨트롤_메시지를_전송한다(t *testing.T) {
	...
	body := msg.sentComplex[0].Content
	// Phase 3 chunk 3B-1: sticky 본문이 "super-session 진행 중"으로 변경됨
	if !strings.Contains(body, "super-session 진행 중") || !strings.Contains(body, "0건") {
		t.Errorf("expected 'super-session 진행 중 · 0건', got %q", body)
	}
}
